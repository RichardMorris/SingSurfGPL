 ...
 */
public class CRe  extends UnaryFunction  implements DiffRulesI,MatrixFunctionI {
    ...
    public CRe(XJep djep, MatrixFactoryI mfac) {
        this.djep = djep;
    ...
    private Object re(Object param1)  throws EvaluationException {
        if(param1 instanceof VectorI)
        {
            return re((VectorI) param1);
        }
        throw new EvaluationException("Real mul both arguments should be vectors");
    }

    private Object re(VectorI p) throws EvaluationException {
        return p.getEle(0);
    }
    ...
    public Dimensions calcDims(Dimensions... inDims) throws ParseException {
        ...
        if(!inDims[0].is1D() || inDims[0].getFirstDim() > 3 ) 
            throw new ParseException("Complex re requires 2 or 3 dimension arguments found "+inDims[0]);
